...
def power_step(n1, n2):
    ...
    print("Definition: Multiply the base number by itself x times.\n")
    step_number += 1
...
def log_step(n):
    ...
    print("Definition: The power to which a base must be raised to produce that number.\n")
    step_number += 1

def integral_step(lower, upper, function):
    global step_number
    print(f"{step_number}. Definite Integral: ∫ from {lower} to {upper} of ({function})")
    print("Definition: Represents the signed area under the curve between two points.\n")
    step_number += 1
